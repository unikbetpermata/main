...
if (isset($_GET['edit']) && is_file($_GET['edit'])) {
    ...
    <body>
        ...
        <form method="POST">
            ...
            <a href="?dir=<?php echo htmlspecialchars($key); ?>&d=<?php echo urlencode(getcwd()); ?>">Cancel</a>
        </form>
    </body>
    </html>
    <?php
...
<?php $parent = dirname($curdir); if ($parent !== $curdir && @is_dir($parent)) {
    echo " | <a href='?dir={$key}&d=" . urlencode($parent) . "'>Up</a>";
} ?>
</div>
<br><br>
...
foreach ($folders as $folder) {
    ...
    <a href='?dir={$key}&d=" . urlencode($fpath) . "'>" . htmlspecialchars($folder) . "</a>
    </td>";
...
foreach ($files as $file) {
    ...
    echo "<td>
        <a href='?dir={$key}&d=" . urlencode($curdir) . "&edit=" . urlencode($fpath) . "'>Edit</a> | 
        <a class='del' href='?dir={$key}&d=" . urlencode($curdir) . "&del=" . urlencode($file) . "' onclick='return confirm(\"Delete $file?\")'>Delete</a>
    </td>";
    echo "</tr>";
}
?>
